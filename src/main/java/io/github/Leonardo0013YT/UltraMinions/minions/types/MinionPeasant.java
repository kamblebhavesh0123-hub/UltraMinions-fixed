...
public class MinionPeasant extends Minion {
         ...
         if (l != null) {
            ...
            if (lo != null) {
               ...
               if (mt != null) {
                  ...
                  if (bl != null) {
                     if (!this.plugin.getAdm().isStackable(bl)) {
                        armor.setItemInHand(new ItemStack(Material.BONE_MEAL, 1));
                     ...
                     List<Location> mf = this.getArroundRandomReady(spawn);
   ...
   public boolean check(Location spawn) {
      ...
      for(Vector l : this.farmers) {
         ...
         if (!lo.getBlock().getType().equals(Material.GRASS_BLOCK) && !lo.getBlock().getType().equals(Material.SHORT_GRASS) && !lo.getBlock().getType().equals(Material.DIRT) && !lo.getBlock().getType().equals(Material.FARMLAND)) {
            yes = false;
         }
      }
   ...
   public Location checkFamerDirt(Location loc) {
      for(Vector l : this.farmers) {
         ...
         if (lo.getBlock().getType().equals(Material.DIRT) || lo.getBlock().getType().equals(Material.SHORT_GRASS) || lo.getBlock().getType().equals(Material.valueOf("GRASS_BLOCK"))) {
            return lo;
         }
      }
   ...
   public List<Location> getArroundRandomReady(Location loc) {
      ...
      int selected = ThreadLocalRandom.current().nextInt(this.farmers.size());
      lo.add((Vector)this.farmers.get(selected));
      lo1.add((Vector)this.block.get(selected));
